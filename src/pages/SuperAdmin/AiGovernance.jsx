  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-neutral-400">Monitor platform-wide LLM token usage costs and manage versioned sub-agent prompt deployments.</p>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-neutral-800 pb-3">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${
            activeTab === 'analytics' ? 'bg-amber-500 text-neutral-950 shadow-md shadow-amber-500/10' : 'text-neutral-400 hover:text-white bg-neutral-900'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${
            activeTab === 'prompts' ? 'bg-amber-500 text-neutral-950 shadow-md shadow-amber-500/10' : 'text-neutral-400 hover:text-white bg-neutral-900'
          }`}
        >
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {aiData?.modelAggregates?.slice(0, 3).map((agg, idx) => (
                  <div key={idx} className="p-5 rounded-2xl bg-neutral-900/90 border border-neutral-800 space-y-2">
                    <span className="text-xs font-semibold text-neutral-400 uppercase tracking-wider">{agg.provider}</span>
                    <h3 className="text-xl font-bold text-white">{agg.model}</h3>
                    <div className="flex justify-between text-xs font-semibold text-neutral-300 pt-2 border-t border-neutral-800">
                    ...
                    {aiData?.items?.map((item) => (
                      <tr key={item.id} className="hover:bg-neutral-800/40">
                        ...
                        <td className="px-6 py-4">
                          ...
                          <span className="text-xs text-neutral-500 block">{item.provider}</span>
                        </td>
                        ...
                        <td className="px-6 py-4 text-xs text-neutral-400">{new Date(item.createdAt).toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {activeTab === 'prompts' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-neutral-900/90 border border-neutral-800 rounded-2xl p-5 space-y-4">
            <h3 className="text-sm font-bold text-white uppercase tracking-wider text-neutral-400">Agent Catalog</h3>
            {loadingPrompts ? (
              <RefreshCw className="w-6 h-6 text-amber-400 animate-spin mx-auto my-6" />
            ) : catalog.length === 0 ? (
              <p className="text-xs text-neutral-500">No agents found in Python service.</p>
            ) : (
                ...
                {catalog.map((ag, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="font-mono text-neutral-400 mt-1">Key: {ag.prompt_key}</div>
                    </button>
                  );
                })}
              </div>
          ...
          {/* Versions List */}
          <div className="lg:col-span-2 bg-neutral-900/90 border border-neutral-800 rounded-2xl p-6 space-y-4">
            ...
            {!selectedAgentId ? (
              <p className="text-sm text-neutral-500 py-12 text-center">Select an agent from the catalog to view prompt version history.</p>
            ) : promptVersions.length === 0 ? (
              <p className="text-sm text-neutral-500 py-12 text-center">No prompt versions found.</p>
            ) : (
              <div className="space-y-4">
                {promptVersions.map((v, idx) => (
                  <div key={idx} className="p-4 rounded-xl bg-neutral-800/60 border border-neutral-700/50 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      {!v.is_active && (
                        <button
                          ...
                          className="px-3 py-1 bg-amber-500 hover:bg-amber-400 text-neutral-950 font-bold text-xs rounded-lg"
                        >
                          Activate Version
                        </button>
                      )}
                    </div>
                    <pre className="text-xs text-neutral-300 font-mono bg-neutral-950 p-3 rounded-lg overflow-x-auto whitespace-pre-wrap max-h-48">
                      {v.prompt}
                    </pre>
                  </div>
                ))}
