  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-neutral-400">Configure global SaaS subscription plans, plan entitlement limits, add-on top-up packs, and AI activity consumption multipliers.</p>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-neutral-800 pb-3">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${
            activeTab === 'plans' ? 'bg-amber-500 text-neutral-950 shadow-md shadow-amber-500/10' : 'text-neutral-400 hover:text-white bg-neutral-900'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${
            activeTab === 'addons' ? 'bg-amber-500 text-neutral-950 shadow-md shadow-amber-500/10' : 'text-neutral-400 hover:text-white bg-neutral-900'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${
            activeTab === 'rules' ? 'bg-amber-500 text-neutral-950 shadow-md shadow-amber-500/10' : 'text-neutral-400 hover:text-white bg-neutral-900'
          }`}
        >
      ...
      {activeTab === 'plans' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {plans.map((plan) => (
            <div key={plan.id} className="p-6 rounded-2xl bg-neutral-900/90 border border-neutral-800 flex flex-col justify-between space-y-4">
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-neutral-500">Order: {plan.sortOrder}</span>
                </div>
              ...
              <div className="space-y-3 pt-3 border-t border-neutral-800">
                <h4 className="text-xs font-bold text-neutral-400 uppercase tracking-wider">Plan Entitlements</h4>
                <div className="space-y-2">
                  {plan.entitlements?.map((ent) => {
                    const isEditing = editingEntitlement?.planId === plan.id && editingEntitlement?.key === ent.key;
                    return (
                      <div key={ent.id} className="flex items-center justify-between text-xs p-2 rounded-lg bg-neutral-800/60 border border-neutral-700/40">
                        <span className="text-neutral-300 font-medium truncate">{ent.key}</span>
                        {isEditing ? (
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              className="w-20 bg-neutral-900 text-white px-2 py-0.5 rounded border border-amber-400 text-xs"
                            />
                            <button
      ...
      {/* TAB 2: ADD-ONS */}
      {activeTab === 'addons' && (
        <div className="bg-neutral-900/90 border border-neutral-800 rounded-2xl p-6 overflow-hidden">
          <table className="w-full text-left text-sm text-neutral-300">
            ...
            <tbody className="divide-y divide-neutral-800/60">
              {addOns.map((addon) => (
                <tr key={addon.id} className="hover:bg-neutral-800/40">
                  ...
                  <td className="px-6 py-4 text-xs font-semibold text-neutral-400">{addon.billingBasis}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <table className="w-full text-left text-sm text-neutral-300">
            ...
            <tbody className="divide-y divide-neutral-800/60">
              {taskRules.map((rule) => {
                ...
                return (
                  <tr key={rule.id} className="hover:bg-neutral-800/40">
                    ...
                    <td className="px-6 py-4">
                      {isEditing ? (
                        <input
                          ...
                          className="w-24 bg-neutral-800 text-white px-2 py-1 rounded border border-amber-400 text-xs"
                        />
                      ) : (
                        <span className="text-amber-400 font-bold">{rule.taskUnits} units</span>
                      )}
                    </td>
                    <td className="px-6 py-4 text-xs text-neutral-400">{rule.customerExplanation || '—'}</td>
                    <td className="px-6 py-4 text-right">
                      {isEditing ? (
                        <button
                          onClick={() => handleSaveRule(rule.activityType, editingRule.taskUnits)}
                          className="px-3 py-1 bg-emerald-500 text-neutral-950 font-bold text-xs rounded-lg"
                        >
                          Save
                        </button>
                      ) : (
                        <button
                          ...
                          className="px-3 py-1 bg-neutral-800 hover:bg-neutral-700 text-amber-400 text-xs rounded-lg border border-neutral-700"
                        >
                          Edit Rate
                        </button>
                      )}
