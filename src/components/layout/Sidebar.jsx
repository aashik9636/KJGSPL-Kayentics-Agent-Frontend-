...
export default function Sidebar() {
  ...
  return (
    <aside ref={sidebarRef} className={`bg-white flex flex-col h-full shrink-0 z-10 relative transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] ${collapsed ? 'w-20' : 'w-64'} pt-2 pb-6 rounded-tr-[36px] rounded-br-[36px] shadow-[4px_0_24px_rgba(0,0,0,0.02)] border-r border-gray-100`}>

      {/* Floating Expand/Collapse Toggle Button */}
      <button
        onClick={() => setCollapsed(!collapsed)}
        className="absolute top-8 -right-3 w-6 h-6 bg-white border border-gray-200 shadow-sm rounded-full flex items-center justify-center text-gray-400 hover:text-[#6c48ff] hover:border-[#6c48ff] z-50 transition-all duration-200"
      ...
      <div className={`h-20 flex items-center sidebar-logo relative ${collapsed ? 'justify-center px-0' : 'px-6 justify-start'}`}>
        <Link to="/" className="flex items-center gap-3 w-full h-full">
          {collapsed ? (
            ...
            <img src="/kaynetics-logo-primary.svg" alt="Kaynetics Logo" className="w-[170px] h-auto object-contain" />
          )}
        </Link>
      </div>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-hide py-2 space-y-5">
        {navSections.map((section) => (
          <div key={section.title} className={`nav-group ${collapsed ? 'px-3' : 'px-5'}`}>
            {!collapsed && (
              <span className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2 block px-3">
                {section.title}
              </span>
            )}
              ...
              {section.items.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center ${collapsed ? 'justify-center px-0' : 'gap-3.5 px-3.5'} py-2.5 rounded-2xl transition-all duration-200 font-semibold text-[13px] ${
                      isActive
                        ? 'bg-purple-50/80 text-[#6c48ff] font-bold shadow-xs'
                        : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'
                    }`}
                  >
                    <span className={`transition-colors ${isActive ? 'text-[#6c48ff]' : 'text-gray-400'}`}>
                      {item.icon}
                    </span>
                    {!collapsed && (
      ...
      <div className={`mt-2 ${collapsed ? 'px-3' : 'px-5'}`}>
        {user?.role === 'SUPER_ADMIN' && (
          <Link
            to="/superadmin"
            className={`mb-3 flex items-center ${collapsed ? 'justify-center p-2.5' : 'px-3.5 py-2.5'} rounded-xl bg-amber-500/10 text-amber-600 hover:bg-amber-500/20 font-bold text-xs border border-amber-500/30 transition-all`}
        ...
        <div className={`${collapsed ? 'bg-transparent' : 'bg-gray-50/80'} rounded-2xl ${collapsed ? 'p-0' : 'p-3'} flex flex-col gap-2 transition-all border border-gray-100`}>
          ...
          {collapsed ? (
            ...
            <button
              onClick={handleLogout}
              className="w-full flex items-center justify-center gap-2 py-1.5 rounded-xl text-[11px] font-bold text-gray-500 hover:text-red-600 hover:bg-red-50 transition-all border border-transparent hover:border-red-100"
            >
